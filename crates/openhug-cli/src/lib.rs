...
mod tests {
    ...
    fn resolves_default_and_cli_overrides_without_mutating_config() {
        ...
        unsafe {
            std::env::remove_var("OPENHUG_URL");
            std::env::remove_var("OPENHUG_TOKEN");
        }
    ...
    fn temp_store() -> (ConfigStore, TempDir) {
        ...
        (
            ConfigStore {
                config_path: dir.path().join("openhug/config.json"),
                legacy_token_path: dir.path().join("openhug/token"),
            },
            dir,
        )
    }
    ...
    #[test]
    fn parse_repo_accepts_owner_name_only() {
        assert_eq!(parse_repo("alice/demo").unwrap(), ("alice", "demo"));
        assert!(parse_repo("alice").is_err());
        assert!(parse_repo("alice/demo/extra").is_err());
    }

    #[test]
    fn pattern_matches_supports_suffix_and_prefix_rules() {
        assert!(pattern_matches(
            "*.safetensors",
            "weights/model.safetensors"
        ));
        assert!(!pattern_matches("*.safetensors", "weights/model.bin"));
        assert!(pattern_matches("weights", "weights/model.bin"));
        assert!(!pattern_matches("weights", "other/model.bin"));
    }

    #[test]
    fn urlencoding_encodes_path_segments() {
        assert_eq!(urlencoding("main"), "main");
        assert_eq!(urlencoding("a b"), "a%20b");
        assert_eq!(urlencode_path("a b/c.d"), "a%20b/c.d");
    }

    #[test]
    fn enforce_secure_token_transport_blocks_remote_http() {
        assert!(
            enforce_secure_token_transport("https://hub.example.com", Some("oh_x"), false).is_ok()
        );
        assert!(
            enforce_secure_token_transport("http://127.0.0.1:3000", Some("oh_x"), false).is_ok()
        );
        assert!(
            enforce_secure_token_transport("http://hub.example.com", Some("oh_x"), false).is_err()
        );
        assert!(
            enforce_secure_token_transport("http://hub.example.com", Some("oh_x"), true).is_ok()
        );
        assert!(enforce_secure_token_transport("http://hub.example.com", None, false).is_ok());
    }

    #[test]
    fn validate_config_rejects_unsupported_versions() {
        let mut config = sample_config();
        config.version = 99;
    ...
    #[test]
    fn validate_config_rejects_unnormalized_entries() {
        let mut config = sample_config();
        config.servers.insert(
            "Bad".to_string(),
            ServerProfile {
    ...
    #[test]
    fn resolve_connection_honors_temporary_server_url() {
        let config = sample_config();
        let resolved = resolve_connection(
            &config,
            &ConnectionOptions {
    ...
    #[test]
    fn resolve_connection_combines_saved_server_with_env_token() {
        let _guard = ENV_TEST_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let config = sample_config();
        let previous_token = std::env::var("OPENHUG_TOKEN").ok();
        unsafe {
            std::env::set_var("OPENHUG_TOKEN", "oh_env");
        }
        let resolved = resolve_connection(&config, &ConnectionOptions::default()).unwrap();
        assert_eq!(resolved.url, "https://hub.example.com");
        assert_eq!(resolved.token.as_deref(), Some("oh_env"));
        unsafe {
            if let Some(value) = previous_token {
                std::env::set_var("OPENHUG_TOKEN", value);
            } else {
                std::env::remove_var("OPENHUG_TOKEN");
            }
        }
    ...
    #[test]
    fn server_registry_mutations_update_default_and_credentials() {
        ...
        store
            .mutate(|config| {
                ...
                config.default_server = Some("home".to_string());
                Ok(())
            })
            .unwrap();
        ...
        store
            .mutate(|config| {
                config.default_server = Some("work".to_string());
                Ok(())
            })
            .unwrap();

        let config = store.load().unwrap();
        assert_eq!(config.default_server.as_deref(), Some("work"));

        store
            .mutate(|config| {
                config.servers.remove("home");
                Ok(())
            })
            .unwrap();
        let config = store.load().unwrap();
        ...
        store
            .mutate(|config| {
                let profile = config.servers.remove("work").unwrap();
                config.servers.insert("office".to_string(), profile);
                config.default_server = Some("office".to_string());
                Ok(())
            })
            .unwrap();
        let config = store.load().unwrap();
        assert!(config.servers.contains_key("office"));
        assert!(!config.servers.contains_key("work"));

        store
            .mutate(|config| {
                config.servers.get_mut("office").unwrap().token = None;
                Ok(())
            })
            .unwrap();
        let config = store.load().unwrap();
        assert!(config.servers["office"].token.is_none());
    }

    #[test]
    fn legacy_token_migration_creates_default_server() {
        let _guard = ENV_TEST_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let (store, _dir) = temp_store();
        unsafe {
            std::env::remove_var("OPENHUG_URL");
        }
        fs::create_dir_all(store.legacy_token_path.parent().unwrap()).unwrap();
        assert!(!store.config_path.exists());
        fs::write(&store.legacy_token_path, "oh_legacy\n").unwrap();
        let config = store.load().unwrap();
        assert_eq!(config.default_server.as_deref(), Some(DEFAULT_SERVER_NAME));
        assert_eq!(
            config.servers[DEFAULT_SERVER_NAME].token.as_deref(),
            Some("oh_legacy")
        );
        assert!(!store.legacy_token_path.exists());
    }
